		...
		for(int centroidIndex = 0 ; centroidIndex <centroids.size() ;centroidIndex++)
		{
			...
			if (counter == 0)
			{
				...
				for (int t = 0; t < K; t++)
				{
					...
					x = ((double) (rand() % magicNum) / magicNum) * (scaledMaxX - scaledMinX) + scaledMinX;
					y = ((double) (rand() % magicNum) / magicNum) * (scaledMaxY - scaledMinY) + scaledMinY;
					centroids[t]-> x = x / magicNum;
					centroids[t]-> y = y / magicNum;
				}
				random_restart++;
				again = true;
				//cout << "random restart id=" << random_restart <<endl;
...
double distanceBetweenPoint(Point* a,Point* b)
{
	double result = sqrt(  pow(abs(a->x - b->x),2) + pow(abs(a->y - b->y ),2) );
	return result;
}

double findMinDistanceBetweenCentroid(int K)
{
	...
	for(int i=0; i< K ;i++)
	{
		for(int j=1;j<K; j++)
		{
			if(j!=i)
			{
				double val = distanceBetweenPoint(centroids[i],centroids[j]);
...
void kMeans(int K)
{
	if(K != 1)
	{
		for(int i=0 ; i<RANDOM_ITERATION ;i++)
		{
			...
			clusterize();
			double minDis =  findMinDistanceBetweenCentroid(K);
			if(minDis > bestDis)
			{
			...
			centroids.clear();
			cout << "iter=" << i << "  result"<<bestDis << endl;
		}
